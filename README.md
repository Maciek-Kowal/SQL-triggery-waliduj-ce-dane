...
&#x20; \* `IBAN\_kraje.sql` - Uniwersalna funkcja walidująca rachunki międzynarodowe.

&#x20; \* `IBAN\_polska.sql` - Wersja zoptymalizowana wyłącznie dla polskich rachunków.

&#x20; \* `TriggerPESEL.sql` - Definicja tabeli, trigger walidacji krzyżowej oraz zapytania testowe.

\* `MySQL/`

&#x20; \* `IBAN\_mysql.sql` - Funkcja walidująca polski IBAN.

&#x20; \* `czy\_pesel\_mysql.sql` - Funkcja sprawdzająca sumę kontrolną PESEL.
...
\* \*\*Automatyzacja:\*\* Wyzwalacze samoczynnie wyłapują niespójności podczas operacji `INSERT` i `UPDATE` bez konieczności ręcznego sprawdzania.

\* \*\*Elastyczność i Wydajność:\*\* Zastosowanie dwóch podejść do walidacji IBAN (rozwiązanie uniwersalne vs zoptymalizowane)
